...
class SportsActivityService extends ActivityService
{
    public function create($user_id, $activity_category, $activity_title, $year, $activity_description, $link) {
        ...
        if ($activity_category === 'international') {           
            ...
            $points = 1;         
        } elseif ($activity_category === 'gto') {
            $points = 1;         
        }

        // Создайте запись в базе данных (предполагается, что у вас есть подключение к БД)
        $sql = "INSERT INTO `sports_activity` (`activity_id`, `user_id`, `points`, `activity_category`, `title`, `year`, `preview_text`, `link`) VALUES (NULL, '$user_id', '$points', '$activity_category', '$activity_title', '$year', '$activity_description', '$link')";
